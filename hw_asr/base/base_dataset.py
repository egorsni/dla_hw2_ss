...
#         index = self._sort_index(index)
    ...
    def __getitem__(self, ind):
        ...
        res = {'target_id': data_dict['target_id']}
        for key in data_dict.keys():
            if key == 'target_id': continue
            pref = key[:-5]
            audio_path = data_dict[key]
            audio_wave = self.load_audio(audio_path)
            audio_wave, audio_spec = self.process_wave(audio_wave)

            res[pref+'_'+"audio"]= audio_wave
#             res[pref+'_'+"spectrogram"]= audio_spec
            res[pref+'_'+"duration"]= audio_wave.size(1) / self.config_parser["preprocessing"]["sr"]
            res[pref+'_'+"audio_path"]= audio_path
        return res
